...
parser = argparse.ArgumentParser(description="test_000020_s verification script")
...
if "stsz_0000000000001dbe" in atom_list[16]:
	...
	if atom[3]["SampleCount"] == 26:
		...
		print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if atom[4]["EntrySize_0"] == 378:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if atom[5]["EntrySize_1"] == 343:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if atom[6]["EntrySize_2"] == 284:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if atom[7]["EntrySize_3"] == 254:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if atom[8]["EntrySize_4"] == 254:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
...
print("*"*40)
print("description:")
print("""A stereo IAMF stream encoded using 20 ms Opus frames which
encodes a 500 ms sine wav with a frequency of 1000 Hz.""")
print("is_valid: True")
print("primary_tested_spec_sections: ['3.5/audio_roll_distance', '3.9/Audio Frame OBU Syntax and Semantics', '3.11.1/OPUS Specific', '3.11.1/num_samples_per_frame', '6.2.2/roll']")
print("*"*40)
