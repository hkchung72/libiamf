...
parser = argparse.ArgumentParser(description="test_000025_s verification script")
...
if "CodecConfigOBU_1" in obu_list[1]:
	...
	if obu.get("codec_config") is not None:
		...
		if obu["codec_config"].get("decoder_config_opus") is not None:
			pass_count += 1
			no_of_check_items += 1
			if obu["codec_config"]["decoder_config_opus"].get("version") == 16:
				pass_count += 1
			else:
				frame = inspect.currentframe()
				print("failure line is #%d."%(frame.f_lineno))
...
print("primary_tested_spec_sections: ['3.10', '3.13.1/version']")
print("*"*40)
