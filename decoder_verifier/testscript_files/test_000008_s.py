...
parser = argparse.ArgumentParser(description="test_000008_s verification script")
...
no_of_check_items += 1
if "stts_000000000000854b" in atom_list[11]:
	pass_count += 1
	atom = atom_list[11]["stts_000000000000854b"]
	no_of_check_items += 1
	if atom[0]["Version"] == 0:
		pass_count += 1
	else:
...
no_of_check_items += 1
if "stsc_0000000000008563" in atom_list[12]:
	pass_count += 1
	atom = atom_list[12]["stsc_0000000000008563"]
	no_of_check_items += 1
	if atom[0]["Version"] == 0:
		pass_count += 1
	else:
...
no_of_check_items += 1
if "stco_000000000000857f" in atom_list[13]:
	pass_count += 1
	atom = atom_list[13]["stco_000000000000857f"]
	no_of_check_items += 1
	if atom[0]["Version"] == 0:
		pass_count += 1
	else:
...
no_of_check_items += 1
if "stsz_0000000000008593" in atom_list[14]:
	pass_count += 1
	atom = atom_list[14]["stsz_0000000000008593"]
	no_of_check_items += 1
	if atom[0]["Version"] == 0:
		pass_count += 1
	else:
...
if "SyncOBU_4" in obu_list[4]:
	...
	if obu.get("sync_array") is not None:
		...
		if sync_array[0].get("obu_data_type") == 0: # OBU_DATA_TYPE_SUBSTREAM
			...
			print("failure line is #%d."%(frame.f_lineno))
		no_of_check_items += 1
		if sync_array[0].get("reinitialize_decoder") == True:
			pass_count += 1
		else:
			frame = inspect.currentframe()
			print("failure line is #%d."%(frame.f_lineno))
...
print("*"*40)
print("description:")
print("""A stereo IAMF stream which is invalid because
`reinitialize_decoder` in the Sync OBU must be false for
Magic Code `version` 0.""")
print("is_valid: False")
print("primary_tested_spec_sections: ['3.12/reinitialize_decoder']")
print("*"*40)
