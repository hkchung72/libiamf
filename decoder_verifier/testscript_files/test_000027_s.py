...
parser = argparse.ArgumentParser(description="test_000027_s verification script")
	...
	if obu.get("codec_config") is not None:
		...
		if obu["codec_config"].get("decoder_config_opus") is not None:
			...
			if obu["codec_config"]["decoder_config_opus"].get("input_sample_rate") == 48000:
				...
				print("failure line is #%d."%(frame.f_lineno))
			no_of_check_items += 1
			if obu["codec_config"]["decoder_config_opus"].get("output_gain") == 1:
				pass_count += 1
			else:
				frame = inspect.currentframe()
				print("failure line is #%d."%(frame.f_lineno))
...
print("primary_tested_spec_sections: ['3.10', '3.13.1/output_gain']")
print("*"*40)
